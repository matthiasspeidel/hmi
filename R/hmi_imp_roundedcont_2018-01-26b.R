...
#' @param PSI A data.frame with the variables explaning the latent rounding tendency G.
...
imp_roundedcont <- function(y_df, X, PSI, rounding_degrees = NULL){
  ...
  tmp <- which(unlist(lapply(X1_stand, get_type)) == "intercept")

  X1_stand <- X1_stand[, c(tmp, (1:ncol(X1_stand))[-tmp])]
...
Consider specifying the variable to be continuous via list_of_types (see ?hmi).\n")
  ...
  betastart <- as.vector(reg_2$coef)
  sigmastart <- stats::sigma(reg_2)
  ...
  mu_g <- gamma1_hat * (as.matrix(MM_y_on_x_2) %*% beta_hat) +
    as.matrix(PSI[, vars_in_psi, drop = FALSE], nrow = nrow(PSI)) %*% gamma_hat
  mu_y <- as.matrix(MM_y_on_x_2) %*% beta_hat
  ...
  y_lower <- (decomposed_y[indicator_precise, "precise"] -
                half_interval_length[as.numeric(as.character(g))] - mean_of_y_precise)/sd_of_y_precise + 1

  y_upper <- (decomposed_y[indicator_precise, "precise"] +
                half_interval_length[as.numeric(as.character(g))] - mean_of_y_precise)/sd_of_y_precise + 1

  g_upper <- bounds_for_g_hat[as.numeric(as.character(g)) + 1]
  ...
  rounded <- rep(TRUE, length(g))

  counter <- 0
  while(any(rounded)){
    counter <- counter + 1
#print(paste("!!!DEBUG!!!", sum(rounded),"\n"))
