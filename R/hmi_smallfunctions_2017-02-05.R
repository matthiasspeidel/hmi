...
get_type <- function(variable){
  ...
  if(is.interval(variable)){
    ...
    if(sum(decompose.interval(variable)$precise %% 5 == 0, na.rm = TRUE)/
...
split.interval <- function(interval){

  #if it is no interval object, just return the object
  if(!is.interval(interval)) return(interval)
...
#' (if it is something else, it is returned unchanged)
#' @return a n times 2 matrix. The first column is the lower bound, the second the upper bound.
sna.interval <- function(x){

  #if it is no interval object, just return the object
  if(!is.interval(x)) return(x)
...
'+.interval' <- function(interval, x){
  tmp <- split.interval(interval) + split.interval(x)
  as.interval(lower = tmp[, 1], upper = tmp[, 2])
}
