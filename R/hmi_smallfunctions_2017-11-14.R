...
#' to be easely visible in the plot.
#' @param x In its most save way, \code{x} is an object from class \code{interval}
#' and joinly used with a second \code{interval} object \code{y}. If no \code{y} is given,
...
#' @param data If \code{x} is a \code{fomula}, it has to be a data.frame or matrix with columnames
...
#' (as such factors might generat linear dependend variables).
#' @param X A n times p data.frame with p fixed (or random) effects variables.
...
#' esential to look a chain free from autocorelation.
...
#' expected value. The difference between both arithmetic means is devided
...
#' If the number of iterations and cylces is large, click through all traceplots
#' can be interminable.
