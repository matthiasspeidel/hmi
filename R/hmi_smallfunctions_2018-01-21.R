...
suggest_rounding_degrees <- function(x){
  ...
  if(!(is.vector(interval) | is_interval(interval))){
  ...
  if(length(tab) == 0){
    ...
    candidates_values <- values[values * tab/ nrow(data) > 2]
...
#' are observed.
#' @param A numeric vector with the presumed rounding degrees. Or a list with rounding degrees,
