...
wrapper <- function(data, model_formula = NULL, parameter_list = NULL,
                    n.iter = 100, M = 10, n.chains = 3, burn.in = 1/3, max.iter = 5000){
  ...
  if(!is.null(model_formula)){
    ...
    # check if this multilevel_target_varnamerealy occurs in the dataest
    ...
    # ---------X variables
    fixedeffects_varname <- all.vars(delete.response(terms(model_formula)))
    # --check if intercept is present
    fixed_intercept_exist <- attributes(delete.response(terms(model_formula)))$intercept
    ...
    nearly_randomeffects_varname <- strsplit(as.character(lme4::findbars(model_formula)), "\\|")[[1]][1]

    #MS: take all variable.names (which are seperated by +)
    #MS: later it might be worth to expand it to interactions etc.
    nearly2_randomeffects_varname <- strsplit(nearly_randomeffects_varname, "\\+")[[1]]

    #MS: remove spaces
    randomeffects_varname <- gsub(" ", "", nearly2_randomeffects_varname)
    ...
    random_intercept_exists <- !"0" %in% randomeffects_varname
    ...
    # make shure, the data will have a "Intercept" variable.
    ...
    if(fixed_intercept_exists | random_intercept_exists | sum(constant_variables) == 1){
      ...
      fixedeffects_varname[fixedeffects_varname == "1"] <- randomintercept_varname

      # Replace the "1" in randomeffects_varname by the name of the intercept variable.
      randomeffects_varname[randomeffects_varname == "1"] <- randomintercept_varname
...
#
  ...
  #MS: Nonresponse-Matrix aufbauen:
  ...
  while(any(missing_rates > 0) & counter <= ncol(my_data) + 10){
    ...
    if(tmp_type == "binary"){
      ...
      imp <- impsyn_bin(y_variable_name = tmp_variable, data_org_bin_imp = tmp_data,
    ...
    if(tmp_type == "cont"){
      ...
      if(is.null(clID_varname) || !(clID_varname %in% names(tmp_data)) || tmp_variable != multilevel_target_varname){
        ...
        #HIER!!!
        imp <- impsyn.cont(y_variable_name = tmp_variable, data.org.cont.imp = tmp_data,
                          cond = parameter_list[[tmp_variable]]$cond,
                          mn = parameter_list[[tmp_variable]]$mn,
                          max_se = parameter_list[[tmp_variable]]$max_se,
...
#Gibbs Sampler fuer random coefficitens (also random intercepts und random slopes)
       ...
       imp <- imp_multi(y_imp_multi = tmp_data[, tmp_variable],
                        X_imp_multi = tmp_X,
                        Z_imp_multi = tmp_Z,
                        clID = tmp_data[, clID_varname],
                        n.iter = n.iter, M = M, n.chains = n.chains,
                        burn.in = burn.in,
                        max.iter = max.iter,
                        allowed_max_value = parameter_list[[tmp_variable]]$allowed_max_value,
                        allowed_max_variable = parameter_list[[tmp_variable]]$allowed_max_variable,
  ...
  while(number.produced.datasets < M & counter < ntries){
    ...
    for(l2 in incomplete.variables){
      ...
      if(tmp_type == "binary"){
        ...
        imp <- impsyn.bin(y_variable_name = l2, data_org_bin_imp = my_data,
