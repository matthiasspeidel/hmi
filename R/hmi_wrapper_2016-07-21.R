...
wrapper <- function(data, model_formula = NULL, parameter_list = NULL,
  ...
  if(!is.null(model_formula)){
    ...
    fixed_intercept_exist <- attributes(delete.response(terms(lme4::nobars(model_formula))))$intercept
...
#Gibbs Sampler fuer random coefficitens (also random intercepts und random slopes)
    ...
    if(only_one_variable_to_impute){
      ...
      midsobj <- list(call = call, data = my_data,
                      ...
                      form = form, post = post, seed = NA, iteration = max.iter,
                      lastSeedValue = .Random.seed, chainMean = colMeans(imp_hmi[[tmp_variable]]),
                      chainVar = apply(imp_hmi[[tmp_variable]], 2, var), loggedEvents = loggedEvents)
